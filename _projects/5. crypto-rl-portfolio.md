...
$$
r_t = \log\left(1 + \mathbf{w}_t^\top \mathbf{R}_{t+1}\right) - c \cdot \|\mathbf{w}_t - \mathbf{w}_{t-1}\|_1
$$
...
- **Turnover limit:** $\|\mathbf{w}_t - \mathbf{w}_{t-1}\|_1 \leq 0.30$ (daily cap)
...
$$
Q_{\text{target}} = r + \gamma \, Q_{\text{target}}(s', \argmax_{a'} Q_{\text{online}}(s', a'))
$$
